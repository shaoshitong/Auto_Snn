...
from lib.multi_iter_network import merge_layer, filename
...
parser.add_argument('--config_file', type=str, default='train_c10_hr.yaml',
...
def neg_smooth_cross(pred, gold, output, smoothing=0.1, *args, **kwargs):
    ...
    with torch.no_grad():
        one_hot = torch.where(torch.eq(out_one_hot.unsqueeze(-1), gold.unsqueeze(-1)), torch.softmax(pred, dim=1),
                              one_hot)
    log_prob = torch.nn.functional.log_softmax(pred, dim=1)
    return torch.nn.functional.kl_div(input=log_prob, target=one_hot, reduction='none').sum(dim=-1).mean()
...
if __name__ == "__main__":
    ...
    dict_list4 = dict(params=params4,
                      weight_decay=yaml['optimizer'][yaml['optimizer']['optimizer_choice']]['weight_decay'])
