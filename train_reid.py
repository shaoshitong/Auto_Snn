...
def train(model, optimizer, scheduler, data, yaml, epoch, criterion_loss,mAP):
    ...
    if isinstance(scheduler, torch.optim.lr_scheduler.MultiStepLR):
        ...
        scheduler(epoch)
    print(f"The train prec1 is {round(total_prec1/len,3)}%,prec5 is {round(total_prec5/len,3)}%,loss is {round(total_loss/len,4)}")
...
if __name__ == "__main__":
    ...
    if yaml['data'] == 'cuhk03':
        ...
        criterion_loss= make_loss(yaml,num_classes)
