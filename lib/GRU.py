...
class MultiAttention(nn.Module):
    def __init__(self,n_model,embed_dim,embed_kv,n_head,bn_size , growth_rate,use_att ,num_index,width,height):
    ...
    def forward(self,x):
        ...
        if self.use_att==True:
            y = x
            x = F.interpolate(x, (self.width, self.height), mode="nearest")
            x=x.view(b,c,-1)
            ...
            att=torch.softmax(F.dropout(torch.matmul(k,v.permute(0,1,3,2))/math.sqrt(k.shape[-1]),training=self.training,p=0.00),dim=1)
            x=F.interpolate(torch.matmul(att,q).permute(0,2,1,3).contiguous().view(b,c,self.width,self.height),(h,w))+y
...
class DenseLayer(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate, class_fusion, width,height, cat_x, cat_y,x,y):
        ...
        if width%7==0:
            ...
            nums_head=4
        if class_fusion == 0:
            self.add_module("attn",MultiAttention(num_input_features,width*height,func_div(width*height,nums_head,nums_head),nums_head,bn_size,growth_rate,use_att,cat_x,width,height))
