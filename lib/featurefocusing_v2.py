...
def block_conv(eq_feature, groups=1):
    model1 = nn.Sequential(nn.BatchNorm2d(eq_feature * 3),
                           nn.LeakyReLU(1e-2),
                           nn.Conv2d(eq_feature * 3, int(eq_feature // groups) * groups, (3, 3), padding=1, stride=1,
                                     groups=groups),
                           nn.LeakyReLU(1e-1),
                           nn.Conv2d(int(eq_feature // groups) * groups, eq_feature, (3, 3), padding=1, stride=1),
                           )
    model3 = nn.Sequential(nn.Conv2d(eq_feature * 3, eq_feature, (1, 1), padding=0, stride=1, groups=groups), )
    model2 = nn.Sequential(block_ad(eq_feature * 3, eq_feature))
...
class feature_norm_layer(nn.Module):
    def __init__(self, eq_feature, groups=1):
        ...
        self.block_conv1, self.block_conv2, self.block_conv3 = block_conv(self.eq_feature, groups=groups)
        self.norm_conv = norm_layer

    def forward(self, x):
        x = (self.block_conv1(x) + self.block_conv3(x)) * self.block_conv2(x)
        return x
...
class Feature_parallel(nn.Module):
    def __init__(self, feature_list, latest=False):
        ...
        self.block_pre = feature_norm_layer(eq_feature=self.feature_list[0], groups=self.feature_list[0])
        self.block_conv = \
            nn.ModuleList([nn.Sequential(*[nn.BatchNorm2d(self.feature_list[0]),
                                           nn.ReLU(inplace=True),
                                           nn.Conv2d(self.feature_list[0], self.feature_list[0], stride=_pair(1),
                                                     kernel_size=(3, 3), padding=1), ]) for _ in range(2)])
        # if latest == False:
        #     self.out_conv = nn.Sequential(nn.BatchNorm2d(self.feature_list[0]),
        #                                   nn.Conv2d(self.feature_list[0], self.feature_list[1], stride=2,
        #                                             kernel_size=(2, 2), ))
        # else:
        #     self.out_conv = nn.Sequential(nn.BatchNorm2d(self.feature_list[0]),
        #                                   nn.Conv2d(self.feature_list[0], self.feature_list[0], stride=1,
        #                                             kernel_size=(1, 1), ))
    ...
    def forward(self, x_list):
        ...
        group_input = self.block_pre(torch.cat(tuple(f_list), dim=1))
...
class Feature_forward(nn.Module):
    def __init__(self, feature_list, p=0.2):
        ...
        self.feature_list2 = copy.deepcopy(feature_list)
        ...
        self.conv_list = nn.ModuleList([nn.Conv2d(i, self.feature_list2[-1], _pair(self.feature_list2[-1]//i), stride=_pair(self.feature_list2[-1]//i)) for i in self.feature_list2[:-1]])
        self.result_conv=nn.ModuleList([nn.Conv2d(self.feature_list2[-1]*2,self.feature_list2[-1],(1,1),(1,1)) for i in range(len(self.feature_list2)-2)])
    def forward(self, x_lists):
        begin = []
        # 64,16,16 128,8,8 256,4,4
        for i, x_list in enumerate(x_lists):
            begin.append(self.dropout(self.Feature_parallel[i](x_list)))
            begin[-1] = self.conv_list[i](begin[-1])
        for i in range(len(begin)-1):
            begin[i+1]=(begin[i+1]+begin[i]/2)
        return begin[-1]
