...
from reid_utils.datasets.itools import mkdir_if_missing, write_json, read_json
from reid_utils.datasets.base import BaseImageDataset
...
class CUHK03(BaseImageDataset):
    ...
    def __init__(self, root='/data/cuhk03_release', split_id=0, cuhk03_labeled=False,
    ...
    def _check_before_run(self):
        ...
        if not osp.exists(self.raw_mat_path):
            raise RuntimeError(
                "'{}' is not available".format(self.raw_mat_path))
        ...
        def _extract_img(name):
            ...
            for campid, camp_ref in enumerate(mat[name][0]):
                ...
                for pid in range(num_pids):
                    ...
                    assert len(
                        img_paths) > 0, "campid{}-pid{} has no images".format(campid, pid)
            ...
            for i, (campid, pid, img_paths) in enumerate(meta_data):
                ...
                if [campid, pid] in test_split:
                    for img_path in img_paths:
                        camid = int(osp.basename(img_path).split(
                            '_')[2]) - 1  # make it 0-based
